...
def nid(name: str, suffix: str):
    return "0x"+binascii.b2a_hex(bytes(reversed(hashlib.sha1((name+suffix).encode()).digest()[:4]))).decode().upper()

assert nid("_Znwj", "PAF") == "0xFC5CD359"

def main():
    outyml = sys.argv[1]
    emd_filename = sys.argv[2]
    libraries = loadEmd(emd_filename)
